...
export const registerUser = createAsyncThunk(
  'auth/register',
  async ({ email, password, name }: RegisterData, { rejectWithValue }) => {
    try {
      const response: AuthResponse = await request(`${BASE_URL}/auth/register`, {
        method: 'POST',
        headers: {
...
export const loginUser = createAsyncThunk(
  ...
  async ({ email, password }: LoginData, { rejectWithValue }) => {
    try {
      const response: AuthResponse = await request(`${BASE_URL}/auth/login`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
...
export const refreshToken = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      const response: AuthResponse = await request(`${BASE_URL}/auth/token`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      return rejectWithValue((error as Error).message || 'Ошибка обновления токена');
    }
  }
);
  ...
  async (email: string, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        (error as Error).message || 'Ошибка отправки email для восстановления'
      );
    }
  }
);
...
export const resetPassword = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      return rejectWithValue((error as Error).message || 'Ошибка сброса пароля');
    }
  }
);
...
export const checkAuth = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      const response: AuthResponse = await requestWithAuth(`${BASE_URL}/auth/user`, {
        method: 'GET',
      });
      ...
      localStorage.removeItem('accessToken');
      localStorage.removeItem('refreshToken');
      return rejectWithValue((error as Error).message || 'Ошибка проверки токена');
    }
  }
);
...
export const updateUser = createAsyncThunk(
  'auth/updateUser',
  async ({ name, email, password }: UpdateUserData, { rejectWithValue }) => {
    try {
      const response: AuthResponse = await requestWithAuth(`${BASE_URL}/auth/user`, {
        method: 'PATCH',
        headers: {
      ...
      return rejectWithValue(
        (error as Error).message || 'Ошибка обновления данных пользователя'
      );
    }
  }
);

export const canAccessResetPassword = (): boolean => {
...
export const requestWithAuth = async (
  ...
  try {
    ...
    if (
      ...
      try {
        const refreshToken = localStorage.getItem('refreshToken');
        const refreshResponse: AuthResponse = await request(`${BASE_URL}/auth/token`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
